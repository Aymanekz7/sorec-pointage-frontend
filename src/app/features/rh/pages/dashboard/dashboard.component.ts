...
export class DashboardComponent implements AfterViewInit, OnDestroy {
  ...
  selectedDepartment = '';
  departmentRows: Attendance[] = [];
  followUpDepartment = 'all';
  followUpSearch = '';
  addFollowUpOpen = false;
  newFollowUpType: 'department' | 'employee' = 'department';
  newFollowUpValue = '';
  savedFollowUps: Array<{ type: 'department' | 'employee'; label: string; value: string }> = [];
  departments: DepartmentCardStats[] = [];
  private allAttendances: Attendance[] = [];
  private viewReady = false;

  get insightCards(): Array<{
    title: string;
    value: string | number;
    note: string;
    subtitle: string;
    ...
    if (!this.isRhView) {
      ...
      const late = this.getLateRowsCount();
      const totalHours = this.getTotalWorkedHours();
      const presenceRate = total ? (presents / total) * 100 : 0;
      const absenceRate = total ? (absents / total) * 100 : 0;

      return [
        {
          title: 'Taux de presence',
          value: `${presenceRate.toFixed(1).replace('.', ',')}%`,
          note: `${presents} collaborateurs presents`,
        ...
        },
        {
          ...
          value: `${absenceRate.toFixed(1).replace('.', ',')}%`,
          note: '',
          subtitle: `${absents} collaborateurs concernes`,
          icon: 'alert' as const,
  ...
  get managerAlerts() {
    return this.isRhView
      ? this.recentAlerts
      : this.recentAlerts.filter(alert => alert.department === this.sessionService.getDepartmentName());
  }
  ...
  get presenceBoardRows(): Array<Attendance & { currentWeek: string; previousWeek: string }> {
  ...
  ngOnInit(): void {
    this.attendanceService.getAttendances().subscribe(rows => {
      ...
      if (this.departments.length && !this.selectedDepartment) {
  ...
  loadDepartmentDetails(department: string): void {
    ...
    this.departmentRows = this.filteredDepartmentRows;
  }
  ...
  setNewFollowUpType(type: 'department' | 'employee'): void {
  ...
  addFollowUp(): void {
    ...
    const value = this.newFollowUpType === 'department'
      ? rawValue
      : rawValue.toLowerCase();

    const label = this.newFollowUpType === 'department'
      ? rawValue
      : this.resolveEmployeeLabel(rawValue);

    const exists = this.savedFollowUps.some(item => item.type === this.newFollowUpType && item.value === value);
    if (!exists) {
      this.savedFollowUps = [
        ...this.savedFollowUps,
        { type: this.newFollowUpType, label, value }
      ];
    }
  ...
  activateFollowUp(target: { type: 'department' | 'employee'; value: string }): void {
  ...
  removeFollowUp(target: { type: 'department' | 'employee'; value: string }): void {
    this.savedFollowUps = this.savedFollowUps.filter(item => !(item.type === target.type && item.value === target.value));
  }
  ...
  get availableEmployeeOptions(): Attendance[] {
    ...
    const matches = this.allAttendances.filter(row => {
      ...
      return row.matricule.toLowerCase().includes(query)
  ...
  private renderCharts(): void {
      ...
      series: [
        {
          type: 'column',
          name: 'Presents',
          data: this.isRhView ? this.departments.map(item => item.presents) : this.managerTrendData,
          color: '#b39b57'
        },
        {
          type: 'column',
    ...
    if (!this.isRhView || !this.weekPieContainer?.nativeElement || !this.weekendPieContainer?.nativeElement) {
      return;
    }
  ...
  private get filteredDepartmentRows(): Attendance[] {
    const department = this.followUpDepartment === 'all'
      ? null
  ...
  private resolveEmployeeLabel(value: string): string {
    ...
    const match = this.allAttendances.find(row =>
  ...
  private getTotalWorkedHours(): number {
    return Math.round(this.allAttendances.reduce((total, row) => total + this.toMinutes(row.workedHours), 0) / 60);
  }

  private buildWeekSummary(row: Attendance, index: number, previous = false): string {
    const base = row.status === 'Present'
      ? previous ? '4/5 jours presents' : '5/5 jours presents'
      : row.status === 'Absent'
        ? previous ? '1 absence' : '2 absences'
        : row.status === 'Deplacement'
          ? previous ? '2 jours terrain' : '3 jours terrain'
          : row.status === 'Conge'
            ? previous ? 'Conge planifie' : 'Conge en cours'
            : previous ? '1 motif saisi' : '2 motifs saisis';
